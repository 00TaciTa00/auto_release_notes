...
[![GitHub Forks](https://img.shields.io/github/forks/00TaciTa00/SoSik?style=flat-square)](https://github.com/00TaciTa00/SoSik/network/members)
[![GitHub Issues](https://img.shields.io/github/issues/00TaciTa00/SoSik?style=flat-square)](https://github.com/00TaciTa00/SoSik/issues)
...
- **AI 제공자**: Claude / OpenAI GPT (레포별 선택, API Key 전역 관리)
...
| AI | 추상화 레이어 — Claude / OpenAI GPT 교체 가능 |
...
| 공통 타입 · 에러 · 로거 (`src/shared/`) | 완료 |
| SQLite 스키마 (`src/db/schema.sql`) | 완료 |
| DB CRUD (`src/db/`) | 완료 |
| IPC 핸들러 (`src/main/ipc/`) | 완료 |
| electron-store 암호화 저장 (`src/main/secure.ts`) | 완료 |
| diff 추출 — GitLab/GitHub API · 로컬 git (`src/diff/`) | 완료 |
| 보안 파일 제외 필터 (`src/diff/securityFilter.ts`) | 완료 |
| AI 요약 — Claude · OpenAI (`src/ai/`) | 완료 |
| 웹훅 수신 서버 (`src/main/webhook/server.ts`) | 완료 |
| 시스템 트레이 | 미구현 |
